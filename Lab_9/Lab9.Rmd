...
Expected death is the implied number of lip cancer deaths for a particular region given that region's age structure and the national level age-specific mortality rates for lip cancers. For example, an expected number of deaths of 6 would mean that for that particular regions, we would expect 6 lip cancer deaths if this region were to experience the same age specific mortality rate as the national level.
...
Here are the three models in notation form:
$$
...
Now we set up the model in R and stan (corresponding stan file are in the same folder):  
...
```{r}
# Run the models
model1 <- stan(data = stan_data,here("lab_9/lab9.stan"))
model2<- stan(data = stan_data,here("lab_9/lab9_2.stan"))
model3<- stan(data = stan_data,here("lab_9/lab9_3.stan"))
```
...
First we generate the dataset for the plots:
...
Model 2 has more uncertainty compared to model 3, we have pooling in model 3 thus we expect it has less uncertainty
